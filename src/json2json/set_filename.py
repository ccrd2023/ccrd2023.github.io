...
for a in os.walk(sys.path[0]):
    ...
    for b in a3:
        ...
        a2.append(clean_filename('%s.json'%('-'.join([m for m in [t['title'],t['date'],'-'.join(t['authors'])]if m])),128))
    a2=['%s.json'%(v)for v in arfn([v.split('.json')[0].strip()for v in a2])]
    n=0
    for b in a3:
        nn+=1
        ...
        if not os.path.exists(dn:='%s/%s'%(a[0],a2[n])):
            f=open(pa:='%s/%s'%(a[0],b),'r');t=eval(f.read());f.close()
            f=open(dn,'w+');f.write(repr(t));f.close()
        n+=1
